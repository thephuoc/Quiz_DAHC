...
function upsertReports(indexItems) {
    ...
    indexItems.forEach((item) => {
        ...
        const candidateKey = `${item?.user?.fullName || 'Unknown'}|${item?.user?.birthDate || ''}`;
...
ipcMain.handle('import-candidates-excel', async () => {
    try {
        ...
        const dateIdx = header.findIndex(h => h.includes('sinh'));
        const unitIdx = header.findIndex(h => h.includes('don') || h.includes('đơn') || h.includes('vi') || h.includes('vị'));

        if (nameIdx < 0 || dateIdx < 0) {
            return { success: false, error: 'Khong tim thay cot "Ho ten" hoac "Ngay sinh" trong file Excel.' };
        }
